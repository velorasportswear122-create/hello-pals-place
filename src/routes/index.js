...
export const Route = createFileRoute("/")({
    head: () => ({
        meta: [
            ...
            {
                name: "description",
                content: "اختر بين قسم التمليك وقسم الإيجار، اعرض وحدتك بالصور والفيديو، وتواصل بأمان عبر إدارة المنصة.",
            },
    ...
    return (<AppShell>
      <main className="relative min-h-screen">
        ...
        <div className="mt-6 grid grid-cols-2 gap-4">
          <SectionCard to="sale" img={saleImg} title="تمليك" sub="بيع وشراء العقارات" tone="gold"/>
          <SectionCard to="rent" img={rentImg} title="إيجار" sub="تأجير واستئجار العقارات" tone="blue"/>
        </div>

        <section className="mt-6 rounded-3xl border border-border bg-card p-5">
          <h2 className="text-center text-base font-bold">لماذا تختار منصتنا ؟</h2>
          <ul className="mt-5 grid grid-cols-4 gap-2">
            {features.map((f) => (<li key={f.title} className="flex flex-col items-center gap-2 text-center">
        ...
        <Link to="/subscribe" className="mt-6 flex items-center justify-between rounded-2xl px-6 py-4 text-primary-foreground shadow-[var(--shadow-soft)]" style={{ backgroundImage: "var(--gradient-gold)" }}>
...
function SectionCard({ to, img, title, sub, tone, }) {
    return (<Link to="/account" search={{ section: to }} className="group relative flex h-64 flex-col justify-end overflow-hidden rounded-3xl border border-border" style={{ backgroundImage: tone === "gold" ? "var(--gradient-gold)" : "var(--gradient-blue)" }}>
      <img src={img} alt={title} loading="lazy" width={800} height={1000} className="absolute inset-x-0 top-0 h-2/3 w-full object-cover transition-transform duration-500 group-hover:scale-105"/>
      ...
      <div className="relative z-10 p-4 text-center text-foreground">
        ...
        <span className="mx-auto mt-3 flex size-8 items-center justify-center rounded-full bg-primary text-primary-foreground">
